...
#define ARBITRARY_THREAD_QUEUE 255
...
restartThread:
    ...
    subQueue = (subQueue + 1) % superThreads;
    numInQueue--;
    pthread_mutex_unlock(&queueMTX);
...
initialReadLabel:
    ...
    case PUT_REQUEST:
        ...
        putExpectedContinueLabel:
            memset(goodResponse, 0, BUFFER_LENGTH);
            checkpoint = putExpectedContinue;
            POLL(thisPoll, thisConn, checkpoint);
    ...
    case APPEND_REQUEST:
        ...
        appendExpectedContinueLabel:
            memset(goodResponse, 0, BUFFER_LENGTH);
            checkpoint = appendExpectedContinue;
            POLL(thisPoll, thisConn, checkpoint);
            n = read(connfd, finalBuffer, BUFFER_LENGTH - 1);
